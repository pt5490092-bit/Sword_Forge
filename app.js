(function(){
  function show(title, detail){
    ...
    v.innerHTML='<div class="empty" style="text-align:left">'+
      '<b>'+title+'</b><br><br>'+
      '<code style="font-size:12px;word-break:break-word">'+
      String(detail).replace(/[&<>]/g,function(c){
        return {'&':'&amp;','<':'&lt;','>':'&gt;'}[c];})+'</code></div>';
  }
  if(!window.addEventListener){ window.__sfShow=show; return; }
  window.addEventListener('error', function(e){
    show('Something went wrong starting the app.',
         (e.message||'script error')+(e.filename?' \u2014 '+e.filename+':'+e.lineno:''));
  });
  window.addEventListener('unhandledrejection', function(e){
    show('Something went wrong starting the app.',
         (e.reason&&e.reason.message)||e.reason||'unknown');
  });
  window.__sfShow=show;
})();
"use strict";
...
function fail(title, detail){
  if(window.__sfShow) return window.__sfShow(title, detail);
  view.innerHTML='<div class="empty" style="text-align:left"><b>'+title+'</b><br><br>'+
    esc(detail)+'</div>';
}

/* Fetch one data file and explain precisely what happened if it fails. */
function grab(rel){
  var url=new URL(rel, document.baseURI).href;
  return fetch(url).then(function(r){
    var ct=(r.headers&&r.headers.get('content-type'))||'';
    if(!r.ok) throw new Error(url+'\n\nThe server answered '+r.status+' '+
      (r.statusText||'')+'.\nThat file is probably not uploaded, or sits at a '+
      'different path.');
    if(ct.indexOf('html')>-1) throw new Error(url+'\n\nThe server returned a web '+
      'page instead of data, which usually means the file is missing and the host '+
      'served a 404 page.');
    return r.json();
  });
}
...
function boot(){
  ...
  var watchdog=setTimeout(function(){
    if(!settled) fail('The scriptures are taking a long time to load.',
      'The data files may be very large on a slow connection, or the request '+
      'may be blocked. Base address: '+document.baseURI);
  }, 25000);

  Promise.all([grab('assets/data/meta.json'), grab('assets/data/bible.json')])
    .then(function(res){
    ...
    .catch(function(e){
      settled=true; clearTimeout(watchdog);
      fail('Could not load the scriptures.',
        ((e&&e.message)||'unknown error')+
        '\n\nIf you opened this file directly from your computer, the browser '+
        'blocks reading local files. Serve the folder over http instead \u2014 for '+
